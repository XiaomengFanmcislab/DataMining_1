...
def knn_missing_filled(k = 3, dispersed = True):
    ...
    x_train = data_lost4[data_lost4.price.notnull()].loc[:,'points'].values.reshape(-1,1)
    y_train = data_lost4[data_lost4.price.notnull()].loc[:,'price'].values.reshape(-1,1)
    print(len(x_train))
    print(len(y_train))
...
min_temp, Q1, median, Q3, max_temp, lower_limit, upper_limit = fiveNumber(points_num_list)
print(att+' five number:')
print(min,Q1,median,Q3,max,lower_limit,upper_limit)
...
# print(price_np)
# plt.boxplot(price_np, labels=['price'])
# plt.show()

# plt.boxplot(points_np, notch = False, labels = ['points'])

# plt.show()

# n, bins, patches = plt.hist(x=price_np, bins='auto', color='#607c8e',
#                             alpha=0.7, rwidth=2)
# plt.show()
...
data_lost4.to_csv('data_knn_2.csv')
